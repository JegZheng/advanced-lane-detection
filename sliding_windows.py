...
def draw_lines(left_fit, right_fit,Minv,perspective,undistorted):
    # Create an image to draw the lines on
    warp_zero = np.zeros_like(perspective)#.astype(np.uint8)
    color_warp = np.dstack((warp_zero, warp_zero, warp_zero))

    ploty = np.linspace(0, perspective.shape[0] - 1, perspective.shape[0])
...
def final_viz(undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
    ...
    """
    # Avg curvature
    avg_cur = (left_curve + right_curve) / 2

    # Generate x and y values for plotting
    ploty = np.linspace(0, undist.shape[0]-1, undist.shape[0])
    left_fitx = left_fit[0]*ploty**2 + left_fit[1]*ploty + left_fit[2] - vehicle_offset
    right_fitx = right_fit[0]*ploty**2 + right_fit[1]*ploty + right_fit[2] - vehicle_offset
